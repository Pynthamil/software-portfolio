...
export default function Navbar() {
  ...
  // Handle opening <details> when hash changes or on initial landing with hash
  useEffect(() => {
    const handleHash = () => {
      const hash = window.location.hash.slice(1);
      if (hash) {
        window.dispatchEvent(new CustomEvent("nav-target", { detail: { id: hash } }));
      }
    };
  ...
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    if (href.startsWith("/#") || href.startsWith("#")) {
      ...
      // If currently on homepage, dispatch event to switch page, open details, and smooth scroll
      if (pathname === "/") {
        e.preventDefault();
        window.dispatchEvent(new CustomEvent("nav-target", { detail: { id } }));
  ...
  const handleLogoClick = () => {
    if (pathname === "/") {
      window.dispatchEvent(new CustomEvent("nav-target", { detail: { id: "top" } }));
